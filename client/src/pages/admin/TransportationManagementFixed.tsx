...
export default function TransportationManagement() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: TransportationFormValues) => {
      const response = await apiRequest("/api/transportation", {
        method: "POST",
        data,
      });
      return response;
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: TransportationFormValues & { id: number }) => {
      const { id, ...rest } = data;
      const response = await apiRequest(`/api/transportation/${id}`, {
        method: "PATCH",
        data: rest,
      });
      return response;
  ...
  const handleEditClick = (transport: Transportation) => {
    ...
    editForm.reset({
      ...
      durationId: transport.durationId,
      passengerCapacity: transport.passengerCapacity,
      baggageCapacity: transport.baggageCapacity,
      price: transport.price,
      discountedPrice: transport.discountedPrice,
      imageUrl: transport.imageUrl || "",
      features: Array.isArray(transport.features) ? transport.features.join(", ") : "",
      withDriver: transport.withDriver,
      available: transport.available,
      pickupIncluded: transport.pickupIncluded,
