...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Setup visa and nationality routes
  setupVisaRoutes(app, storage, isAdmin);
  
  // Setup export/import routes
  setupExportImportRoutes(app, storage, isAdmin);
    ...
    try {
        ...
        for (const country of countries) {
          ...
          if (!existing) {
            await storage.createCountry({
              ...country,
              imageUrl: `https://images.unsplash.com/400x300/?${country.name}`,
              active: true
            });
          ...
          if (country) {
            ...
            if (!existing) {
              await storage.createCity({
                ...
                description: city.description,
                imageUrl: `https://images.unsplash.com/400x300/?${city.name}`,
                active: true
              });
              seedResults.cities++;
            }
      ...
      if (existingHotels.length < 15) {
        ...
        const hotelNames = [
          ...
          'Royal Suites', 'Garden Hotel', 'Beach Resort', 'Heritage Hotel',
          'Modern Tower Hotel', 'Courtyard Hotel', 'Premium Inn'
        ];

        for (let i = 0; i < Math.min(15, hotelNames.length); i++) {
      ...
      if (existingPackages.length < 12) {
        ...
        const packageTypes = [
          ...
          { title: 'Middle East Grand Tour', duration: 15, type: 'Multi-Country' },
          { title: 'Red Sea Diving Package', duration: 6, type: 'Water Sports' },
          { title: 'Holy Land Pilgrimage', duration: 9, type: 'Religious' },
          { title: 'Arabian Peninsula Discovery', duration: 14, type: 'Cultural' }
        ];
        ...
        for (const pkg of packageTypes) {
          ...
          await storage.createPackage({
            ...
            price: Math.floor(Math.random() * 100000) + 50000, // 500-1500 USD in cents
            duration: pkg.duration,
            maxGroupSize: Math.floor(Math.random() * 20) + 10,
            difficulty: ['Easy', 'Moderate', 'Challenging'][Math.floor(Math.random() * 3)],
            includes: ['Accommodation', 'Meals', 'Transportation', 'Guide', 'Entrance Fees'],
      ...
      for (const transport of transportTypes) {
        try {
          await storage.createTransportType({
            ...
            capacity: transport.name.includes('Bus') ? 45 : transport.name.includes('SUV') ? 7 : 4,
            pricePerKm: Math.floor(Math.random() * 300) + 100, // 1-4 USD per km in cents
            active: true
          });
